...
void build_gui()
{
	...
	roll_graph_file = glui->add_rollout( "Graph file", false );
		glui->add_edittext_to_panel( roll_graph_file, "Filename", GLUI_EDITTEXT_TEXT, graph_file);
		glui->add_button_to_panel( roll_graph_file, "Load", 0, load_graph);
		glui->add_button_to_panel( roll_graph_file, "Save", 0, save_graph);
		glui->add_button_to_panel( roll_graph_file, "Clear", 0, clear_graph);
	roll_graph_file->disable();

	roll_graph = glui->add_rollout( "Object", false );
		object_list   = glui->add_listbox_to_panel( roll_graph, "Type:", &object_type);
		company_list  = glui->add_listbox_to_panel( roll_graph, "Company:", &company_type);
		                glui->add_button_to_panel( roll_graph, "Add",    0, add_object);
		remove_object = glui->add_button_to_panel( roll_graph, "Remove", 0, rem_object);
		update_object = glui->add_button_to_panel( roll_graph, "Update", 0, upd_object);
		remove_object->disable();
		update_object->disable();
		object_mod_panel = glui->add_panel_to_panel(roll_graph, "", GLUI_PANEL_NONE);
