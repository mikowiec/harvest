...
typedef ShipX PlayerShipBase;

class PlayerShip : public PlayerShipBase
{
	EventProxy ep;
public:

	PlayerShip(PlayerID id, CompanyID i, const Vector& v)
	 : PlayerShipBase(SillyData(i, Matrix4(v))),
	   ep(EventSystem::get_ref(id))
	{ }
	~PlayerShip() { derr << "ship dying!!!\n"; }
	void think() {
		namespace id = os::event::id;
		os::event::Event e;
	...
	void set_cockpit(bool inside)
	{
		ctrl.inside_cockpit = inside;
	}
};
...
public:
	...
	DummyShip(const Vector& v)
	 : P(SillyData(Punkrocker, Matrix4(v)))
	{
		ctrl.yaw = 0.1;
	}
	void think() { }
...
struct Config {
	   ...
	   td(1)
	{
		...
		level	    = withdefault(cnf, "level", std::string("test_level"));
...
struct Game {
	...
	Game(os::gfx::Gfx& gx) : game(gx), wr(0), mr(0), tr(0), rr(0), pr(0), sr(0) {  }
};
...
void create_objs(Game& go)
{
	...
	for (int i = 0; i < 1; ++i, z_pos -= 30 ) {
		...
		PlayerShip* s = new PlayerShip(go.ply_ids[i], (i == go.local_id) ? Slick : None,
						Point(4089,600,3235));
		derr << "creating ship: " << s << '\n';
...
// Dummyships
	for (int i = 0; i < go.cnf.ship_block; i++) {
...
void game_init(os::gfx::Gfx& gx, Game& go)
{
	debug::debug_disable("sockstream");
	debug::debug_disable("config");
	debug::debug_disable("scenario");
	debug::debug_disable("snd_dummy");
	debug::debug_disable("event");

	go.wr = world::World::create(go.cnf.level);
	go.pr = phys::Engine::create();

	go.local_id_tmp = net::init_game(go.game, go.cnf.run_multi, go.cnf.observer);
...
int foo()
{
	...
	EventProxy ep = EventSystem::get_ref(os::event::System);


	create_objs(*go);
	go->sr = game::scenario::ScenarioMgr::create(go->a_ships[go->local_id]);
	...
	sm.set_camera(c, go->a_ships[go->local_id]->get_velocity());
	go->rr->render(c);
	gx->update_screen();

	os::time::msleep(1000);

	go->game.wait_for_start();
	RelTime prevtime = os::time::get_rel_time();
	...
	timespan real_time = os::time::get_time();

	timers.frame.start();
	RelTime sim_time = 0;
	while (!quit && !exit_now()) {
		long diff = (os::time::get_time() - real_time).approx() / 1000;
		sim_time += diff;
//		derr << "sim_time: " << sim_time << '\n';
		real_time = os::time::get_time();
		...
		{ Profile p(timers.io);
		  ...
		  misc::for_each(seq(go->a_ships), urk());
                ...
                double timediff = (os::time::get_rel_time() - prevtime) * 1e-3;
                prevtime = os::time::get_rel_time();
		float dt = timediff;
		{ Profile p(timers.ai);
			misc::for_each(seq(go->turrets), urk());
			misc::for_each(seq(go->dyn_objs), urk());
		}
//	      	derr << "running phys at: " << sim_time << " for " << diff << '\n';
		{ Profile p(timers.phys);
			go->pr->update_world(sim_time * 1e-3, diff * 1e-3);
		}
		os::time::rel_time_tick(timediff * 1e3);
		...
		{ Profile p(timers.snd);
		  sm.set_camera(cm, go->a_ships[go->local_id]->get_velocity());
		  sm.run();
		}
